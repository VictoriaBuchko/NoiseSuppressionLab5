...
Всі три фільтри помітно відстають від ідеальної траєкторії (чорний пунктир). SMA з вікном 20 точок створює затримку ~0.4 с (20/50 = 0.4 с) і "зрізає кути" на поворотах через усереднення. EMA з α=0.1 має еквівалентне вікно ~20 точок і поводиться схоже. Медіанний фільтр W=21 реагує дещо швидше на зміни напрямку, але через "стрибки" медіани між сусідніми значеннями лінія трохи менш гладка.
 
**Порівняння обробки викидів (Median vs SMA):**
 
Медіанний фільтр повністю ігнорує поодинокі викиди - різкі сірі піки на графіку не впливають на зелену лінію. SMA натомість "розмазує" кожен викид на 20 точок (0.4 с), створюючи хвилеподібні артефакти на синій лінії. Для координатних даних з GPS-приймачів, де викиди трапляються регулярно, це критична різниця.
